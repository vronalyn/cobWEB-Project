   ...
   import { getFirestore, getDocs, collection,onSnapshot, doc, deleteDoc,updateDoc, addDoc } from "https://www.gstatic.com/firebasejs/9.19.1/firebase-firestore.js";
...
querySnapshot.forEach((doc) => {
  const data = doc.data();
  const editButton = `<button type="button" class="btn btn-primary edit" data-id="${doc.id}" data-region="${data.Region}" data-province="${data.Province}" data-municipality="${data.Municipality}" data-poverty-incidence="${data.PovertyIncidence}" data-toggle="modal" data-target="#edit-modal">Edit </button>  `
    ...
    $(document).on('click', '.edit', function() {
      if ($(this).data('id') === `${doc.id}`) {
        ...
        const PovertyIncidence = $(this).data('poverty-incidence');
          ...
          $('#MunicipalityEdit').val(Municipality);
    ...
    $(document).off('click', '#update-button').on('click', '#update-button', function () {
        ...
        const newData = {
          ...
          'PovertyIncidence': PovertyIncidence,
             ...
             db2.collection("2012city").doc(docId).update(newData).then(() => {
            ...
            addDoc(activityLogCollection, {
              ...
              action: `${Municipality} has been Updated`,
              details: `${Municipality} data from 2012 Municipal and City Poverty Dataset has been Updated.`
            }).then(() => {
      ...
      $(document).off('click', '#deleteData').on('click', '#deleteData', function () {
        ...
        console.log('Delete docId:', docId); // Check if Document ID matches with docID
             ...
             db2.collection("2012city").doc(docId).delete().then(() => {
